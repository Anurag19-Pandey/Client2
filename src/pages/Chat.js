...
const Chat = () => {
  ...
  const deleteMsg = (chat_id) => {
    ...
    setLoadMessage(
      loadmessages.map((lmsg) => {
        if (lmsg.chat_id == chat_id) lmsg.message_status = "inactive";
  ...
  const chatWindow = async (id, name, profileImage) => {
    ...
    if (room != id) {
      ...
      setChat(true);
      if (window.innerWidth <= 440) setCanvaShow(true);

      const { data } = await axios.get(
        `${finalUrl}/api/userchat?user_id=${id}&requested_by=admin`
      );
  ...
  const lastMsgDate = (created_at) => {
    const d = new Date(created_at * 1000);
    const completeDate =
      d.getDate() + " " + month[d.getMonth()] + ", " + (d.getFullYear() % 100);
          ...
          {searchArray.length > 0 ? (
            searchArray.map((user, index) => {
              return (
                <>
                  <div
                    className="userQueryList"
                    key={index}
                    onClick={() => {
                      user.message_status = "seen";
                      chatWindow(
                        user.user_user_id,
                        user.full_name,
                        user.profile_image
                  ...
                  >
                    <div className="userImageAndNameCont">
                      <img
                        ...
                        className="userProfileImageInChatInfo"
                      />
                    </div>
                        ...
                        user.media_type == "image" ? (
                          <div className="userLastMessageInfo">
                            ...
                            <div className="userTimeAndUnseenMessageCont">
                              <span className="lastMessagedeliverDate">
                                {lastMsgDate(user.created_at) +
                                  " | " +
            ...
            >
              <RiUserSearchLine style={{ fontSize: "40px" }} />
              &nbsp;No results found
            </p>
          ) : active.length > 0 ? (
            active.map((user, index) => {
              return (
                <>
                  <div
                    className={`userQueryList ${
                      room == user.user_user_id ? "changeBackQuery" : null
                    }`}
                    key={index}
                    onClick={() => {
                      user.message_status = "seen";
        ...
        <div className="canvaContainer">
          <Offcanvas className="h-100 p-0" show={canvaShow} placement="bottom">
            <div className="chatHeaderCanvasContainer">
              ...
              <div
                className="userImagePlaceholderContainer"
                style={{ backgroundImage: `url(${profileImage})` }}
              ></div>
              <p className="chatUserName">{chatName}</p>
            </div>
            <div ref={scrollRef} className="chatBodyCanvasContainer w-100">
                      ...
                      {lmsg.sent_by == "admin" ? (
                        <div className="rightSideChatContainer" key={index}>
                          {lmsg.message_status != "inactive" ? (
                            <div
                              className="deleteMsgContainer"
                              onClick={() => deleteMsg(lmsg.chat_id)}
                            >
                              ...
                              lmsg.media_type == "text" ? (
                                  ...
                                  <div className="message">
                                    <p
                                      ...
                                      dangerouslySetInnerHTML={{
                                        __html: Linkify(lmsg.message),
                                      }}
                                    />
                                    <span className="messageTime">
                                      {handleTime(lmsg.created_at)}
                                    </span>{" "}
                                  </div>
                                </>
                                ...
                                <>
                                  <img
                                    ...
                                    onClick={() => handleShowImage(lmsg.url)}
                                  />{" "}
                                  <p className="messageMediaTime">
                                    {handleTime(lmsg.created_at)}
                                  </p>{" "}
                                </>
                              ) : (
                                <div
                                  className="chatVideo"
                                  onClick={() => handleShowVideo(lmsg.url)}
                                >
                                  {" "}
                                  <video
                                    className="rightChatVideo"
                                    height={"200px"}
                                    width={"200px"}
                                  >
                                    {" "}
          ...
          <>
            ...
            <div className="chatBodyContainer">
              <SnackBar
                ...
                snackBarState={setSnakckbarStatus}
              />
              {chat &&
                loadmessages &&
                loadmessages.map((lmsg, index) => {
                    ...
                    <div>
                      ...
                      {lmsg.sent_by == "admin" ? (
                        <div className="rightSideChatContainer" key={index}>
                          ...
                          <div className="messageInfoContainerRight">
                            <p className="SentByName">
                              You{" "}
                              {lmsg.media_type == "text" &&
                              lmsg.message_status != "inactive" ? (
                                <MdContentCopy
                                  onClick={() => copyText(lmsg.message)}
                                  style={{
                                    color: "#b2b2b2",
                                    cursor: "pointer",
                                  }}
